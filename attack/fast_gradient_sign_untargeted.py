...
class FastGradientSignUntargeted():
    ...
    def perturb(self, original_images, labels, 
        ...
        if random_start:
            ...
            rand_perturb = utils.tensor2cuda(rand_perturb)
        ...
        with torch.enable_grad():
            for _iter in range(self.max_iters):
                ...
                if exclusive:
                    ...
                    loss = F.cross_entropy(nn.Softmax(dim=1)(outputs), labels)
                if reduction4loss == 'none':
                    grad_outputs = utils.tensor2cuda(torch.ones(loss.shape))                    
                else:
                    grad_outputs = None
                grads = torch.autograd.grad(loss, x, grad_outputs=grad_outputs, 
                        only_inputs=True)[0]
                #print(grads.data.cpu().detach().numpy())
                # temp = grads.data.cpu().detach().numpy()
