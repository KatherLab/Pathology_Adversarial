...
def ConcatCohorts_Classic(imagesPath, cliniTablePath, slideTablePath,  label, reportFile, outputPath, csvName, patientNumber = 'ALL', minNumberOfTiles = 0, 
    ...
    if patientNumber == 'ALL':        
        ...
        if additionalData:
            ...
            preproccessDropNo = 0
            preproccessDrop = []
            for tempPi in set(patientList_return) ^ set(patients):
                slideTemp = slideTableList.loc[slideTableList['PATIENT'] == tempPi]
                slideTemp.reset_index(inplace = True, drop = True)
                flag = False
                for item in slideTemp['FILENAME']:
                    if [s for s in additionalData if item in s]:
                        flag = True
                if flag:                                            
                    preproccessDropNo += 1
                    preproccessDrop.append(tempPi)
                else:
                    noSlideNo += 1
                    noSlide.append(tempPi)
                
                    
        print('Dropout {} Patients because of preprocessing steps : {} '.format(preproccessDropNo, preproccessDrop))
        reportFile.write('Dropout {} Patients because of preprocessing steps : {} '.format(preproccessDropNo, preproccessDrop) + '\n')   
        
        print('Dropout {} Patients because of because there is no WSI : {} '.format(noSlideNo, noSlide))
        reportFile.write('Dropout {} Patients because of because there is no WSI : {} '.format(noSlideNo, noSlide) + '\n')  
...
def GetTiles(csvFile, label, maxBlockNum, target_labelDict, test = False, filterPatients = []):
